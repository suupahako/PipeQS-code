...
class GCN(GNNBase):
    ...
    def forward(self, g, feat, bits, epoch, rank, in_norm=None, out_norm=None):
        h = feat
        for i in range(self.n_layers):
            if i < self.n_layers - self.n_linear:
                if self.training and (i > 0 or not self.use_pp):
                    h = ctx.buffer.update(i, h, bits, epoch, rank)
                with ctx.neu_timer.timer(f"forward_{epoch}_{i}"):
                    with ctx.forward_timer.timer(f"forward_{epoch}_{i}"):
                        h = self.dropout(h)
            ...
            if i < self.n_layers - 1:
                with ctx.neu_timer.timer(f"activation_{epoch}_{i}"):
                    with ctx.forward_timer.timer(f"activation_{epoch}_{i}"):
                        if self.use_norm:
                            h = self.norm[i](h)
                        h = self.activation(h)
...
class GraphSAGE(GNNBase):
    ...
    def forward(self, g, feat, bits, epoch, rank, in_deg=None):
        h = feat
        ...
        for i in range(self.n_layers):
            if i < self.n_layers - self.n_linear:
                ...
                h = self.dropout(h)
                with ctx.neu_timer.timer(f"forward_{epoch}__{i}"):
                    with ctx.forward_timer.timer(f"forward_{epoch}__{i}"):
...
class GAT(GNNBase):
    ...
    def forward(self, g, feat, bits, epoch, rank):
        h = feat
        for i in range(self.n_layers):
            if i < self.n_layers - self.n_linear:
                if self.training:
                    if i > 0 or not self.use_pp:
                        h1 = ctx.buffer.update(i, h, bits, epoch, rank)
                    else:
                        h1 = h
