...
class Buffer(object):
    ...
    def init_buffer(self, num_in, num_all, boundary, f_recv_shape, layer_size, use_pp=False, backend='gloo',
        ...
        if backend == 'gloo':
            ...
            self._qt_f_cpu = [None] * self._n_layers
            self._qt_f_cpu_recv = [None] * self._n_layers
            self._qt_g_cpu = [None] * self._n_layers
            self._qt_g_cpu_recv = [None] * self._n_layers
            ...
            for i in range(self._n_layers):
                ...
                for j in range(size):
                    if j == rank:
                        ...
                        tmp_recv_qt_f_cpu.append(None)
                        tmp_recv_qt_g_cpu.append(None)
                        ...
                        # 接收
                        tmp_recv_parm_f_cpu.append(pack_param_tensors(torch.zeros(s_tmp_rmin_cpu, dtype=torch.bfloat16, pin_memory=True), torch.zeros(s_tmp_scale_cpu, dtype=torch.bfloat16, pin_memory=True), torch.zeros(s_tmp_shape_cpu, dtype=torch.int64, pin_memory=True)))
                        tmp_recv_parm_g_cpu.append(pack_param_tensors(torch.zeros(s_tmp_rmin_recv_cpu, dtype=torch.bfloat16, pin_memory=True), torch.zeros(s_tmp_scale_recv_cpu, dtype=torch.bfloat16, pin_memory=True), torch.zeros(s_tmp_shape_recv_cpu, dtype=torch.int64, pin_memory=True)))
                        tmp_recv_qt_f_cpu.append(torch.zeros(s_tmp_qt_f_recv_cpu, dtype=torch.int8, pin_memory=True))
                        tmp_recv_qt_g_cpu.append(torch.zeros(s_tmp_qt_f_cpu, dtype=torch.int8, pin_memory=True))
                ...
                # 接收
                self._parm_f_cpu[i] = tmp_recv_parm_f_cpu
                self._qt_f_cpu[i] = tmp_recv_qt_f_cpu
                if i > 0:
                    self._parm_g_cpu[i] = tmp_recv_parm_g_cpu
                    self._qt_g_cpu[i] = tmp_recv_qt_g_cpu
        ...
        for i in range(self._n_layers):
            ...
            for j in range(size):
                if j == rank:
                    ...
                    # tmp_qt_f_gpu.append(torch.zeros(s_tmp_qt_f_gpu, device='cuda'))
                    # tmp_scale_gpu.append(torch.zeros(s_tmp_scale_gpu, device='cuda'))
                    # tmp_rmin_gpu.append(torch.zeros(s_tmp_rmin_gpu, device='cuda'))
                    # tmp_shape_gpu.append(torch.zeros(s_tmp_shape_gpu, device='cuda'))

                    # # 反向传播的缓冲区刚好跟前向传播相反
                    # tmp_qt_g_gpu.append(torch.zeros(s_tmp_qt_f_recv, device='cuda'))
                    # tmp_g_scale_gpu.append(torch.zeros(s_tmp_scale_recv, device='cuda'))
                    # tmp_g_rmin_gpu.append(torch.zeros(s_tmp_rmin_recv, device='cuda'))
                    # tmp_g_shape_gpu.append(torch.zeros(s_tmp_shape_recv, device='cuda'))

                    # tmp_qt_f_recv.append(torch.zeros(s_tmp_qt_f_recv, device='cuda'))
                    # tmp_scale_recv.append(torch.zeros(s_tmp_scale_recv, device='cuda'))
                    # tmp_rmin_recv.append(torch.zeros(s_tmp_rmin_recv, device='cuda'))
                    # tmp_shape_recv.append(torch.zeros(s_tmp_shape_recv, device='cuda'))

                    # # 反向传播
                    # tmp_qt_g_recv.append(torch.zeros(s_tmp_qt_f_gpu, device='cuda'))
                    # tmp_g_scale_recv.append(torch.zeros(s_tmp_scale_gpu, device='cuda'))
                    # tmp_g_rmin_recv.append(torch.zeros(s_tmp_rmin_gpu, device='cuda'))
                    # tmp_g_shape_recv.append(torch.zeros(s_tmp_shape_gpu, device='cuda'))
        ...
        with true_comm_timer.timer(f'forward_{epoch}_{layer}'):
            with forward_comm_timer.timer(f'_{epoch}_{layer}'):
                ...
                with torch.cuda.stream(self._comm_stream):
                    self.quantization_to_cpu(self, feat, self._qt_f_cpu[layer], self._parm_f_cpu[layer], bits, forward = True)
                    ...
                    for i in range(1, size):
                        ...
                        r1_0 = dist.isend(self._qt_f_cpu[layer][right], tag=tag, dst=right)
                        req_send.append(r1_0)
                        r1_1 = dist.isend(self._parm_f_cpu[layer][right, tag = tag + self.n_layers, dst = right])
                        req_send.append(r1_1)

                        r2_0 = dist.irecv(self._qt_f_cpu_recv[layer][left], tag=tag, src=left)
                        r2_1 = dist.irecv(self._parm_f_cpu_recv[layer][left], tag = tag + self.n_layers, src = left)
                    ...
                    while not req_recv.empty():
                        ...
                        with unpack_timer.timer(f"forward_unpack_{epoch}_{layer}_{idx}"):
                            recv_rmin, recv_scale, recv_shape = unpack_param_tensors(self._parm_f_cpu[layer][idx])

                        
                        self._qt_f_recv[layer][idx] = self._qt_f_cpu_recv[layer][idx].to('cuda')
    ...
    def __qt_backward_gloo_all_to_all(self, grad, layer, tag, bits, epoch, forward=False):
        with true_comm_timer.timer(f'backward{epoch}_{layer}'):
            with backward_comm_timer.timer(f'_{epoch}_{layer}'):
                ...
                self._comm_stream.wait_stream(torch.cuda.current_stream()) 
                with torch.cuda.stream(self._comm_stream):
                    self.quantization_to_cpu(self, grad, self._qt_g_cpu[layer], self._parm_g_cpu[layer], bits, forward = True)
                    ...
                    for i in range(1, size):
                        ...
                        r1_0 = dist.isend(self._qt_g_cpu[layer][right], tag=tag, dst=right)
                        req_send.append(r1_0)
                        r1_1 = dist.isend(self._parm_g_cpu[layer][right, tag = tag + self.n_layers, dst = right])
                        req_send.append(r1_1)

                        r2_0 = dist.irecv(self._qt_g_cpu_recv[layer][left], tag=tag, src=left)
                        r2_1 = dist.irecv(self._parm_g_cpu_recv[layer][left], tag = tag + self.n_layers, src = left)
                        req_recv.put((r2_0, r2_1, left))  
                    ...
                    while not req_recv.empty():
                        ...
                        with unpack_timer.timer(f"forward_unpack_{epoch}_{layer}_{idx}"):
                            recv_rmin, recv_scale, recv_shape = unpack_param_tensors(self._parm_g_cpu[layer][idx])

                        
                        self._qt_g_recv[layer][idx] = self._qt_g_cpu_recv[layer][idx].to('cuda')
                        ...
                        shape = (shape_tensor[0].item(), shape_tensor[1].item())
                        with dq_timer.timer(f"forward_dq_{epoch}_{layer}_{idx}"):
                            self._b_recv[layer][idx] = message_dequantization(self._qt_f_recv[layer][idx], self._scale_recv[layer][idx], self._rmin_recv[layer][idx], shape, bits)
...
#                 #         shape = message_quantization(feat[self._boundary[1]], bits, stochastic=True, 
#                 #                         q_input_holder=self._qt_f_gpu[layer][1], 
#                 #                         q_scale_holder=self._scale_gpu[layer][1], 
#                 #                         rmin_holder=self._rmin_gpu[layer][1])
#                 #     # 打印循环外部的显存使用情况
#                 #     print(f"Iteration {i + 1}:")
#                 #     print_memory_usage("End of iteration")
#                 #     print("----------------------------")
...
#                     #             #         self._qt_f_gpu[layer][right], self._scale_gpu[layer][right], self._rmin_gpu[layer][right], shape = message_quantization(feat[self._boundary[right]], bits, stochastic=True)
#                     #             # dist.barrier()
#                     #             if epoch == 0 or rank == 0:
#                     #                 self._qt_f_gpu[layer][right], self._scale_gpu[layer][right], self._rmin_gpu[layer][right], shape = message_quantization(feat[self._boundary[right]], bits, stochastic=True)
#                     #                 self._shape_gpu[layer][right] = torch.tensor(shape)
                                
#                     #             if rank == 0 and DEBUG:
#                     #                 print(f"forward layer {layer} epoch {epoch} before: {self._qt_f_gpu[layer][right].shape} {self._scale_gpu[layer][right].shape} {self._rmin_gpu[layer][right].shape}")
...
#                     #             if rank == 0 and DEBUG:
#                     #                 print(f"forward layer {layer} epoch {epoch} after: {self._qt_f_gpu[layer][right].shape} {self._scale_gpu[layer][right].shape} {self._rmin_gpu[layer][right].shape}")
...
#                                 # tmp_send_gpu_tensor = []
#                                 pack_tensors_in_place(self.send_cpu_tensor[layer][right],
#                                             self._rmin_gpu[layer][right],
#                                             self._scale_gpu[layer][right],
#                                             self._shape_gpu[layer][right],
#                                             self._qt_f_gpu[layer][right])        
#                                 # print(self.send_cpu_tensor[layer][right])
...
#                                 #         print(f"{k}- Allocated memory: {allocated:.2f} MB, Reserved memory: {reserved:.2f} MB")
#                                 #         self._f_recv[layer][idx] = message_dequantization(self._qt_f_recv[layer][idx], self._scale_recv[layer][idx], self._rmin_recv[layer][idx], shape, bits)
#                                 # dist.barrier()
#                                 self._f_recv[layer][idx] = message_dequantization(self._qt_f_recv[layer][idx], self._scale_recv[layer][idx], self._rmin_recv[layer][idx], shape, bits)
...
#                     #             self._qt_g_gpu[layer][right], self._g_scale_gpu[layer][right], self._g_rmin_gpu[layer][right], shape = message_quantization(grad[self._pl[right]:self._pr[right]], bits, stochastic=True)
#                     #             if rank == 0 and DEBUG:
#                     #                 print(f"backward layer {layer} epoch {epoch} before: {self._qt_g_gpu[layer][right].shape} {self._g_scale_gpu[layer][right].shape} {self._g_rmin_gpu[layer][right].shape}")
...
#                     #             if rank == 0 and DEBUG:
#                     #                 print(f"backward layer {layer} epoch {epoch} after: {self._qt_g_gpu[layer][right].shape} {self._g_scale_gpu[layer][right].shape} {self._g_rmin_gpu[layer][right].shape}")
...
#                             # q_feat, scale, rmin, shape = message_quantization(feat[self._boundary[right]], bits, stochastic=True)

#                             # self._qt_g_gpu[layer][right], self._g_scale_gpu[layer][right], self._g_rmin_gpu[layer][right], shape = message_quantization(feat[self._boundary[right]], bits, stochastic=True)
#                             # 组装为一个tensor
#                             # tmp_send_cpu_tensor = pack_tensors(rmin, scale, torch.tensor(shape), q_feat)
#                             if DEBUG:
...
#                                 # tmp_send_gpu_tensor = []
#                                 pack_tensors_in_place(self.g_send_cpu_tensor[layer][right],
#                                             self._g_rmin_gpu[layer][right],
#                                             self._g_scale_gpu[layer][right],
#                                             self._g_shape_gpu[layer][right],
#                                             self._qt_g_gpu[layer][right])      
#                                 # print(self.g_send_cpu_tensor[layer][right])                          
...
# 反向传播, 不使用量化
    ...
    def quantization_to_cpu(self, feat, qt_cpu, parm_cpu, bits, forward):
        rank, size = dist.get_rank(), dist.get_world_size()
        for i in range(1, size):
            right = (rank + i) % size
            if forward:
            ...
            qt_cpu[right].copy_(qt)
            parm_cpu[right].copy_(parm)
 

    
    # 消息量化
    def quantization(self, feat, qt_gpu, scale_gpu, rmin_gpu, shape_gpu, bits, forward):
        rank, size = dist.get_rank(), dist.get_world_size()
        for i in range(1, size):
            right = (rank + i) % size
            if forward:
            ...
            qt_gpu[right].copy_(qt, non_blocking = True)
            scale_gpu[right].copy_(scale, non_blocking = True)
            rmin_gpu[right].copy_(rmin, non_blocking = True)
            shape_gpu[right].copy_(shape_tensor, non_blocking = True)
            # # Print debug information
            # print(f"Rank: {rank}, Right: {right}, Forward: {forward}")
            # print(f"Quantized Tensor Shape: {qt_gpu[right].shape}")
            # print(f"Scale Shape: {scale_gpu[right].shape}")
            # print(f"Rmin Shape: {rmin_gpu[right].shape}")
            # print(f"Message Shape: {shape}")
    ...
    def update(self, layer, feat, bits, epoch, rank):
        ...
        if FORWARD_QUANTIZATION:
            self.quantization(feat, self._qt_f_gpu[layer], self._scale_gpu[layer], self._rmin_gpu[layer], self._shape_gpu[layer], bits, forward=True)
    ...
    def __grad_hook(self, epoch, layer, bits, transfer_group, update_group, rank):
        def fn(grad):
            ...
            if BACKWARD_QUANTIZATION:
                self.quantization(grad, self._qt_g_gpu[layer], self._g_scale_gpu[layer], self._g_rmin_gpu[layer], self._g_shape_gpu[layer], bits, forward=False)
