...
// min：每个浮点数数据的最小值。
// max：每个浮点数数据的最大值。
// scale：缩放因子，用于量化浮点数数据。
// stochastic：一个布尔值，指示是否使用随机化量化。
// stream：CUDA流，用于在GPU上执行并发操作。
// 其他参数用于控制CUDA内核函数的执行，如线程块的维度等。


// 在压缩算法中，整数流的压缩是通过按照一定规则将浮点数数据转换为整数，并将这些整数流打包成一个长整数来实现的。
// 在这个过程中，我们可以看到以下变化：

// 重新排列： 在压缩过程中，可能会将原始的二维张量 Tensor data 重新排列为一维数组，以便于按顺序处理数据。

// 压缩后的形状： 压缩后的形状可能是一个一维数组，其中包含了压缩后的整数流数据。
// 这个数组的长度取决于压缩算法的实现细节，通常是压缩后的数据流长度。

// 可能的细节： 在压缩过程中，可能会考虑批量处理数据，因此压缩后的形状可能会受到批量大小 N 和数据的特征维度 group_size 的影响。
// 可能会使用循环或者并行处理来处理多个数据块，因此最终形状可能是一个多维数组或者一维数组的集合。
